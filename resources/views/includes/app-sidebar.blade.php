...
@php
    $profileType = auth()->user()->profile?->type;

    $showUserMenu = $profileType == ProfileType::User;
    $showExhibitionMenu = $profileType == ProfileType::Exhibitor;
...
<aside id="logo-sidebar"
    class="fixed left-0 z-40 h-full w-64 -translate-x-full transition-transform sm:translate-x-0 md:top-24 2xl:top-16"
    aria-label="Sidebar">
    <div class="h-full overflow-y-auto bg-brand py-4">
        <a href="/dashboard" class="mb-5 grid items-center ps-2.5 text-center">
            <img src="{{ Vite::asset('resources/images/ictc_logo.svg') }}" class="mx-auto h-auto w-32"
                alt="Flowbite Logo" />
            <span class="self-center whitespace-nowrap text-[0.8rem] font-semibold text-secondary dark:text-white">Events
