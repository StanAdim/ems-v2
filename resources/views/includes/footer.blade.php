<footer class="bg-black">

    <div class="bg-brand">
        <div class="container mx-auto grid grid-cols-1 gap-8 bg-brand py-14 text-white md:grid-cols-3">
            <div class="col-span-2 ml-1">
                <div>
                    <h3 class="text-3xl font-semibold">
                        Stay Updated
                    </h3>
                    <p class="text-lg font-light">
                        Get the latest updates on what’s going on!
                    </p>
                </div>
                @if ($errors->any())
                    <script>
                        document.addEventListener("DOMContentLoaded", function() {
                            window.location.hash = '#subscription-form';
                        });
                    </script>
                @endif
                <form action="{{ route('subscriber.subscribe') }}" method="post">
                    @csrf

                    <div id="subscription-form" class="mt-3 flex flex-wrap gap-2">
                        <div>
                            <input class="h-auto rounded-full px-4 py-2 text-lg text-black placeholder-black"
                                placeholder="Name" type="text" name="name">
                            @error('name')
                                <x-input-error :messages="$message" class="mt-2" />
                            @enderror
                        </div>
                        <div>
                            <input class="h-auto rounded-full px-4 py-2 text-lg text-black placeholder-black"
                                placeholder="Your email address" type="email" name="email" id="email">
                            @error('email')
                                <x-input-error :messages="$message" class="mt-2" />
                            @enderror
                        </div>
                        <div>
                            <button class="h-auto rounded-full bg-primary px-20 py-2 text-lg"
                                type="submit">Subscribe</button>
                        </div>
                    </div>
                </form>
            </div>
            <div class="mx-auto ml-1 mt-auto text-center md:ml-auto">
                <h4 class="mb-1 text-lg font-normal">Connect with us</h4>
                <div class="inline-flex gap-3">
                    <a class="rounded-full p-2 ring-1 ring-white" href="#">
                        <svg class="size-5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg" x="0px"
                            y="0px" width="100" height="100" viewBox="0 0 24 24">
                            <path
                                d="M17.525,9H14V7c0-1.032,0.084-1.682,1.563-1.682h1.868v-3.18C16.522,2.044,15.608,1.998,14.693,2 C11.98,2,10,3.657,10,6.699V9H7v4l3-0.001V22h4v-9.003l3.066-0.001L17.525,9z">
                            </path>
                        </svg>
                    </a>
                    <a class="rounded-full p-2 ring-1 ring-white"
                        href="https://x.com/ict_commission/status/1820362917436412099?s=48">
                        <svg class="size-5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg" x="0px"
                            y="0px" width="100" height="100" viewBox="0 0 24 24">
                            <path
                                d="M 2.3671875 3 L 9.4628906 13.140625 L 2.7402344 21 L 5.3808594 21 L 10.644531 14.830078 L 14.960938 21 L 21.871094 21 L 14.449219 10.375 L 20.740234 3 L 18.140625 3 L 13.271484 8.6875 L 9.2988281 3 L 2.3671875 3 z M 6.2070312 5 L 8.2558594 5 L 18.033203 19 L 16.001953 19 L 6.2070312 5 z">
                            </path>
                        </svg>
                    </a>
                    <a class="rounded-full p-2 ring-1 ring-white"
                        href="https://www.instagram.com/ict_commission_tanzania?igsh=MWc5OW1jemgycW1tcw%3D%3D&utm_source=qr">
                        <svg class="size-5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg" x="0px"
                            y="0px" width="100" height="100" viewBox="0 0 50 50">
                            <path
                                d="M 16 3 C 8.8324839 3 3 8.8324839 3 16 L 3 34 C 3 41.167516 8.8324839 47 16 47 L 34 47 C 41.167516 47 47 41.167516 47 34 L 47 16 C 47 8.8324839 41.167516 3 34 3 L 16 3 z M 16 5 L 34 5 C 40.086484 5 45 9.9135161 45 16 L 45 34 C 45 40.086484 40.086484 45 34 45 L 16 45 C 9.9135161 45 5 40.086484 5 34 L 5 16 C 5 9.9135161 9.9135161 5 16 5 z M 37 11 A 2 2 0 0 0 35 13 A 2 2 0 0 0 37 15 A 2 2 0 0 0 39 13 A 2 2 0 0 0 37 11 z M 25 14 C 18.936712 14 14 18.936712 14 25 C 14 31.063288 18.936712 36 25 36 C 31.063288 36 36 31.063288 36 25 C 36 18.936712 31.063288 14 25 14 z M 25 16 C 29.982407 16 34 20.017593 34 25 C 34 29.982407 29.982407 34 25 34 C 20.017593 34 16 29.982407 16 25 C 16 20.017593 20.017593 16 25 16 z">
                            </path>
                        </svg>
                    </a>
                    <a class="rounded-full p-2 ring-1 ring-white" href="#">
                        <svg class="size-5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg" x="0px"
                            y="0px" width="100" height="100" viewBox="0 0 32 32">
                            <path
                                d="M 16.09375 4 C 11.01675 4 6 7.3833281 6 12.861328 C 6 16.344328 7.9584844 18.324219 9.1464844 18.324219 C 9.6364844 18.324219 9.9199219 16.958266 9.9199219 16.572266 C 9.9199219 16.112266 8.7460938 15.131797 8.7460938 13.216797 C 8.7460938 9.2387969 11.774359 6.4199219 15.693359 6.4199219 C 19.063359 6.4199219 21.556641 8.3335625 21.556641 11.851562 C 21.556641 14.478563 20.501891 19.40625 17.087891 19.40625 C 15.855891 19.40625 14.802734 18.516234 14.802734 17.240234 C 14.802734 15.370234 16 13.558906 16 11.628906 C 16 8.3529063 11.462891 8.94725 11.462891 12.90625 C 11.462891 13.73725 11.5665 14.657063 11.9375 15.414062 C 11.2555 18.353063 10 23.037406 10 26.066406 C 10 27.001406 10.133656 27.921422 10.222656 28.857422 C 10.390656 29.045422 10.307453 29.025641 10.564453 28.931641 C 13.058453 25.517641 12.827078 24.544172 13.955078 20.076172 C 14.564078 21.234172 16.137766 21.857422 17.384766 21.857422 C 22.639766 21.857422 25 16.736141 25 12.119141 C 25 7.2061406 20.75475 4 16.09375 4 z">
                            </path>
                        </svg>
                    </a>
                    <a class="rounded-full p-2 ring-1 ring-white"
                        href="https://youtube.com/@ictcommission5362?si=G0wDIsYYcEfpFmUQ">
                        <svg class="size-5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg" x="0px"
                            y="0px" width="100" height="100" viewBox="0 0 24 24">
                            <path
                                d="M21.582,6.186c-0.23-0.86-0.908-1.538-1.768-1.768C18.254,4,12,4,12,4S5.746,4,4.186,4.418 c-0.86,0.23-1.538,0.908-1.768,1.768C2,7.746,2,12,2,12s0,4.254,0.418,5.814c0.23,0.86,0.908,1.538,1.768,1.768 C5.746,20,12,20,12,20s6.254,0,7.814-0.418c0.861-0.23,1.538-0.908,1.768-1.768C22,16.254,22,12,22,12S22,7.746,21.582,6.186z M10,15.464V8.536L16,12L10,15.464z">
                            </path>
                        </svg>
                    </a>
                    <a class="rounded-full p-2 ring-1 ring-white"
                        href="https://www.linkedin.com/posts/ict-commission-tanzania_tanzaniametaverselecture-metaverse2024-innovation-activity-7225925620038291456-ZI5T?utm_source=share&utm_medium=member_ios">
                        <svg class="size-5 text-white" fill="currentColor" xmlns="http://www.w3.org/2000/svg" x="0px"
                            y="0px" width="100" height="100" viewBox="0 0 50 50">
                            <path
                                d="M 8 3.0097656 C 4.53 3.0097656 2.0097656 5.0892187 2.0097656 7.9492188 C 2.0097656 10.819219 4.59 12.990234 8 12.990234 C 11.47 12.990234 13.990234 10.870625 13.990234 7.890625 C 13.830234 5.020625 11.36 3.0097656 8 3.0097656 z M 3 15 C 2.45 15 2 15.45 2 16 L 2 45 C 2 45.55 2.45 46 3 46 L 13 46 C 13.55 46 14 45.55 14 45 L 14 16 C 14 15.45 13.55 15 13 15 L 3 15 z M 18 15 C 17.45 15 17 15.45 17 16 L 17 45 C 17 45.55 17.45 46 18 46 L 27 46 C 27.552 46 28 45.552 28 45 L 28 30 L 28 29.75 L 28 29.5 C 28 27.13 29.820625 25.199531 32.140625 25.019531 C 32.260625 24.999531 32.38 25 32.5 25 C 32.62 25 32.739375 24.999531 32.859375 25.019531 C 35.179375 25.199531 37 27.13 37 29.5 L 37 45 C 37 45.552 37.448 46 38 46 L 47 46 C 47.55 46 48 45.55 48 45 L 48 28 C 48 21.53 44.529063 15 36.789062 15 C 33.269062 15 30.61 16.360234 29 17.490234 L 29 16 C 29 15.45 28.55 15 28 15 L 18 15 z">
                            </path>
                        </svg>
                    </a>
                </div>
            </div>
        </div>
    </div>

    <div class="mx-auto w-full max-w-screen-2xl">

        <div class="grid grid-cols-2 gap-8 pt-8 md:grid-cols-4 lg:pt-10">
            <a href="/">
                <div class="">
                    <img width="198px" class="" src="{{ Vite::asset('resources/images/white_logo.png') }}"
                        alt="" srcset="">
                </div>
            </a>
        </div>
        <div class="grid grid-cols-2 gap-8 px-5 py-6 md:grid-cols-4 lg:py-8">
            <div>
                <h2 class="mb-1 text-lg font-semibold text-white">Contact Us</h2>
                <ul class="font-light text-white">
                    <li class="">
                        <a href="#" class="hover:underline">27 Ali Hassan Mwinyi Road,</a>
                    </li>
                    <li class="">
                        <a href="#" class="hover:underline">
                            P.O. BoX 70479 <br>
                            11102 DAR ES SALAAM
                        </a>
                    </li>
                    <li class="">
                        <a href="mailto:dg@ictc.go.tz" class="hover:underline">Email: dg@ictc.go.tz</a>
                    </li>
                    <li class="">
                        <a href="tel:+255736848444" class="hover:underline">Phone: +255 7368 48444</a>
                    </li>
                </ul>
            </div>
            <div>
                <h2 class="mb-1 text-lg font-semibold text-white">Useful links</h2>
                <ul class="font-light text-white">
                    <li class="">
                        <a href="https://www.ictc.go.tz/" class="underline hover:text-gray-400" target="_blank">ICT
                            Commission</a>
                    </li>
                    <li class="">
                        <a href="https://www.mawasiliano.go.tz/" target="_blank"
                            class="underline hover:text-gray-400">Ministry of Communication and Information Technology</a>
                    </li>
                    <li class="">
                        <a href="https://iprs.ictc.go.tz/" class="underline hover:text-gray-400" target="_blank">ICT
                            Professional Registration</a>
                    </li>
                </ul>
            </div>
            <div>
                <h2 class="mb-1 text-lg font-semibold text-white">Quick Links</h2>
                <ul class="font-light text-white">
                    <li class="">
                        <a href="/" class="underline hover:text-gray-400">Home</a>
                    </li>
                    <li class="">
                        <a href="https://taic.ictc.go.tz/#price" class="underline hover:text-gray-400">Conference
                            Fees</a>
                    </li>
                    <li class="">
                        <a href="https://taic.ictc.go.tz/#team" class="underline hover:text-gray-400">Speakers</a>
                    </li>
                </ul>
            </div>
            <div>
                <h2 class="mb-8 text-sm font-semibold text-white"></h2>
                <ul class="font-light text-white">
                    <li class="">
                        <a href="https://ictc.go.tz/terms-and-conditions" class="hover:underline">Terms & Conditions</a>
                    </li>
                    <li class="">
                        <a href="https://ictc.go.tz/privacy-policy" class="hover:underline">Privacy Policy</a>
                    </li>
                    <li class="">
                        <a href="https://ictc.go.tz/disclaimers" class="hover:underline">Disclaimer</a>
                    </li>
                </ul>
            </div>
        </div>
    </div>
    {{-- <div class="bg-primary w-full text-center py-5">
        <p class="text-white text-sm">
            Copyright &copy; {{ date('Y') }}, Designed and Developed by ICT Commission
        </p>
    </div> --}}
</footer>
